...
#ifndef _WMRDE_SPATIAL_BLOCK_H_
#define _WMRDE_SPATIAL_BLOCK_H_
...
public:
  ...
  inline void from6x1(const Eigen::Matrix<Real,6,1>& src)
  {
    b0 = src.topRows<3>();
    b1 = src.bottomRows<3>();
  }
...
public:
  ...
  inline Eigen::Matrix<Real,6,6> to6x6() const
  {
    ...
    out.topLeftCorner<3,3>() = B0;
    out.bottomLeftCorner<3,3>() = B1;
    out.topRightCorner<3,3>() = B2;
    out.bottomRightCorner<3,3>() = B3;
    return out;
  }

  inline void from6x6(const Eigen::Matrix<Real,6,6>& src)
  {
    B0 = src.topLeftCorner<3,3>();
    B1 = src.bottomLeftCorner<3,3>();
    B2 = src.topRightCorner<3,3>();
    B3 = src.bottomRightCorner<3,3>();
  }
...
//[R         0]
//[skew(t)*R R]
inline Mat6b HTToPlucker6b(const HTransform& HT)
{
...
//[ R'          0 ]
//[-R'*skew3(t) R']
inline Mat6b invHTToPlucker6b(const HTransform& HT)
{
...
//FOR DEBUGGING
inline HTransform Plucker6bToHT(const Mat6b& P)
{
...
//R2 = M2*P3;
//R3 = M3*P3;
inline Mat6b multMatPlucker6b(const Mat6b& M, const Mat6b& P)
{
  return Mat6b(
      M.B0*P.B0 + M.B2*P.B1,
      M.B1*P.B0 + M.B3*P.B1,
...
//TODO, is this faster?
inline void multMatPlucker6b(const Mat6b& M, const Mat6b& P, Mat6b& R)
{
...
inline Mat6b toSpatialInertia6b(const Real m, const Vec3& c, const Mat3& I)
{
  Mat3 C = skew(c);
  return Mat6b(I + m*(C*C.transpose()), m*C, m*C.transpose(), m*Mat3::Identity());
}

void fromSpatialInertia6b(const Mat6b& Is, Real& m, Vec3& c, Mat3& I)
{
  m = Is.B3(0,0);
  Mat3 C = Is.B2/m;
  c = unskew(C);
